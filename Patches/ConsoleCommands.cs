...
namespace BalaurBohemianBroken.Patches;
...
public class ConsoleCommands {
    ...
    private static void PrintStats(string[] args) {
        ...
        for (int i = 1; i < args.Length; i++) {
            ...
            instance.LogToConsole($"{args[i]}: {tracker.Serialize()} ({tracker.GetStatReadout(true)})");
        }
    }
    ...
    private static void PrintAllTrackers(string[] args) {
        ...
        foreach (KeyValuePair<string, IStat> kvp in ExpandedDeathReports.StatTrackersAll) {
            instance.LogToConsole($"{kvp.Value.name}: {kvp.Value.Serialize()} ({kvp.Value.GetStatReadout(true)})");
        }
    }
}